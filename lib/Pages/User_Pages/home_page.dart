        ...
        builder: (context, state) {
          ...
          if (state is LoadingStatus) {
            ...
            bodyWidget = (index == 2
                ? OriginalPage(
                    name: widget.name,
                    services: BlocProvider.of<HomePageCubit>(context).services,
                    sponsors: BlocProvider.of<HomePageCubit>(context).sponsors,
                  )
                : Screens[index])!;
          } else {
            bodyWidget = ErrorPage(
                imageParh: "assets/photo/NoInternaetAnimation.json",
                upperMessage: "خطا في الانترنت",
                lowerMessage: "اقفل و افتح تاني ");
          }
          return bodyWidget;
        },
      ),
