...
class PostMethods {
  ...
  static Future<bool> updateEmployeeProfile({
    ...
    try {
      final String url =
          'https://mahllola.online/api/employee/editEmployeeProfile/$employeeId';
  ...
  static Future<bool> updateWorkImages({
    ...
    try {
      final String url =
          'https://mahllola.online/api/employee/updateWorksImage/$employeeId';

      FormData formData = FormData.fromMap({
        if (workImageOne != null)
          'works[0][image]': await MultipartFile.fromFile(workImageOne.path,
              filename: 'work_image_1.jpg'),
        if (workImageTwo != null)
          'works[1][image]': await MultipartFile.fromFile(workImageTwo.path,
              filename: 'work_image_2.jpg'),
        if (workImageThree != null)
          'works[2][image]': await MultipartFile.fromFile(workImageThree.path,
              filename: 'work_image_3.jpg'),
        if (workImageFour != null)
          'works[3][image]': await MultipartFile.fromFile(workImageFour.path,
              filename: 'work_image_4.jpg'),
      });
...
//////////////////////////////// Make Order Method /////////////////////////////
  ...
  Future<dynamic> makeOrder(Order order) async {
    ...
    try {
      ...
      if (e.response != null) {
        print(
            'Error making order: ${e.response?.statusCode} - ${e.response?.statusMessage}');
  ...
  Future<bool> updateWorkImage(int userid, File? image, String name) async {
    try {
      final String url =
          'https://mahllola.online/api/employee/updateWorksImage/$userid';
      FormData formData = FormData.fromMap({
        if (image != null)
          '$name': await MultipartFile.fromFile(image.path,
              filename: 'profile_image.jpg'),
      });
      Response response = await dio.post(url, data: formData);
...
//////////////////// Update Work Images for employee ///////////////
  Future<bool> updateProfileInfo(int userid, File? image, String imgName,
      String? name, String? desc, int? price) async {
    try {
      final String url =
          'https://mahllola.online/api/employee/editEmployeeProfile/$userid';
      FormData formData = FormData.fromMap({
        if (name != null) 'name': name,
        ...
        if (image != null)
          '$imgName':
              await MultipartFile.fromFile(image.path, filename: imgName),
      });
    ...
    try {
      final Response response = await dio.post(
        ...
        data: {
          ...
          'date_of_delivery': order.dateOfDelivery,
          'order_descriptions': order.orderDescriptions,
...
////////////////////////////////// Make Employee Profile Method ///////////////////////
  ...
  static Future<bool> createEmployee(
      ...
      FormData? works1,
      FormData? works2,
      FormData? works3) async {
    const String url =
        'https://mahllola.online/api/employee/employeeCompleteData';
...
////////////////////////////////edit Employee Method /////////////////////
  ...
  static Future<void> changeOrderStatus(int orderId, String newStatus) async {
    ...
    try {
      ...
      if (e.response != null) {
        print(
            'Error changing order status: ${e.response?.statusCode} - ${e.response?.statusMessage}');
        print('Response data: ${e.response?.data}');
      } else {
  ...
  static Future<dynamic> changeServiceProviderId(String state) async {
    final String url =
        'https://mahllola.online/api/employee/changeEmployeeStatus/${CacheData.getData(key: "employee_id")}';
  ...
  static Future<dynamic> makeFeedback(
      int empId, String Comment, int rating, int orderId) async {
